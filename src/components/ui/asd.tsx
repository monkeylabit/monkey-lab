...
// mesh-gradient: 4 dark/black points
vec3 meshGradient(vec2 uv){
  vec2 p0=vec2(-0.70,-0.45), p1=vec2(0.75,-0.35), p2=vec2(-0.65,0.65), p3=vec2(0.80,0.55);
  // Converted to grayscale/dark black tones
  vec3 c0=vec3(0.02,0.02,0.02);  // nearly black
  vec3 c1=vec3(0.05,0.05,0.05);  // dark gray
  vec3 c2=vec3(0.01,0.01,0.01);  // pure black
  vec3 c3=vec3(0.04,0.04,0.04);  // charcoal
  float e=2.0;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  // Background base colors - CHANGED TO BLACK/DARK GRAY
  vec3 baseDeep = vec3(0.01,0.01,0.01);
  vec3 baseTint = vec3(0.03,0.03,0.03);
  ...
  // Line colors - CHANGED TO NEUTRAL GRAY/WHITE (so they are visible on black)
  // Giving them a slight cyan/blue tint to match the site's accent, but very desaturated/darker
  vec3 lineThin  = vec3(0.20,0.20,0.25); 
  vec3 lineMajor = vec3(0.30,0.30,0.35);
  ...
  if (ASCII_AMT > 0.001) {
    ...
    // Ascii color - keep a bit of branding (cyan-ish) but subtle
    vec3 asciiColor = mix(vec3(0.2,0.6,0.8), meshGradient(uv), 0.25);
...
export default function ASDBackground({
    className,
    style
}: {
    className?: string;
