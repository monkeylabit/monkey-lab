...
// Declare the extended JSX namespace for the custom shader material
declare global {
    namespace JSX {
    ...
    const getThemeColors = () => {
        switch (theme) {
            case 'dark':
                return {
                    dotColor: '#FFFFFF',
                    bgColor: '#000000', // Ensuring pure black as per previous requests
                    dotOpacity: 0.025
                }
            case 'light':
                return {
...
export const DotScreenShader = () => {
    return (
        <Canvas
            ...
            className="fixed inset-0 w-full h-full pointer-events-none -z-10" // Added classes for fixed background behavior
        >
            <Scene />
        </Canvas>
    )
